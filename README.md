...
3. **roles/** - Отдельные файлы для каждой роли
   - **miner.js** - Майнеры (копают под себя, без ног)
   - **taxi.js** - Такси (доставка майнеров)
   - **courier.js** - Курьеры (транспортировка ресурсов)
   - **builder.js** - Строители (строительство)
   - **repairer.js** - Ремонтники (ремонт структур)
   - **upgrader.js** - Апгрейдеры (апгрейд контроллера)
...
2. Создать файл в папке roles/ с логикой выполнения
3. Обновить spawnerManager для спавна новой роли
4. Добавить импорт и вызов в main.js
...
### 4. Выполнение задач
- Каждая роль имеет свою логику выполнения в отдельном файле
